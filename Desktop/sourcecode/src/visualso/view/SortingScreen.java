		...
		Visualizer main =  new Visualizer(array) {
			...
			public void paintComponent(Graphics g) {
				...
				for (int i = 0; i< array.length;i++) {
					g.setColor(color);
				    g.fillRect(i*ArrayUtils.min(width,60+padding)+(getWidth()-ArrayUtils.min(width,60+padding)*mainArray.length)/2,
				    		   -(int)(getArray()[i] * unitHeight)+ getHeight(),
							   ArrayUtils.min(width-padding,60),
